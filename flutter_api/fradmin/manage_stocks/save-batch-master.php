...
try {
    ...
    $action = strtolower(trim($_POST['action'] ?? $_REQUEST['action'] ?? 'save'));
    $frId = (int) ($_POST['FrId'] ?? $_REQUEST['user_id'] ?? $_REQUEST['FrId'] ?? 0);
    $createdBy = (int) ($_POST['CreatedBy'] ?? $_REQUEST['CreatedBy'] ?? $frId);
    $id = (int) ($_POST['id'] ?? 0);
