...
if ($_POST['action'] == 'Add') {
    ...
    // ðŸ”¹ Helper: Random String Generator
    ...
    try {
        ...
        $fields = [
            ...
            'StockQty', 'MinQty', 'ProdType2', 'Transfer', 'PurchasePrice', 'Unit', 'Division', 'Segment', 'Family',
            'ClassId', 'McDesc', 'BrandDesc', 'Status', 'SrNo','userid'
        ];
        ...
        $userid = $_POST['userid'];
        // ðŸ”¹ INSERT or UPDATE logic
        if (empty($id)) {
            // ðŸŸ¢ INSERT tbl_cust_products2
             ...
             $sql = "INSERT INTO tbl_cust_products2 
                    SET Division='$Division', Segment='$Segment', Family='$Family', ClassId='$ClassId',
                        ...
                        StockQty='$StockQty', TempPrdId='$TempPrdId', MinQty='$MinQty', Photo='$Photo',CreatedBy='$userid'";
           ...
           $sql = "INSERT INTO tbl_cust_products_2025 SET ProdId='$ProdId', 
                     Division='$Division', Segment='$Segment', Family='$Family', ClassId='$ClassId',
                        ...
                        CatId='$CatId', MinPrice='$MinPrice', Status='$Status', SrNo='$SrNo', CreatedDate='$CreatedDate',
                        CgstPer='$CgstPer', SgstPer='$SgstPer', IgstPer='$IgstPer', GstAmt='$GstAmt', ProdPrice='$ProdPrice',
                        CgstAmt='$CgstAmt', SgstAmt='$SgstAmt', IgstAmt='$IgstAmt', BarcodeNo='$BarcodeNo',checkstatus=1,
                        StockQty='$StockQty', TempPrdId='$TempPrdId', MinQty='$MinQty', Photo='$Photo',CreatedBy='$userid',code='$Code2'";
        ...
        if (!$conn->query($sql2)) {
            ...
            // ðŸŸ¢ Update code field
            if (!$conn->query("UPDATE tbl_cust_products_2025 SET code='$Code2' WHERE id='$ProdId'"))
                throw new Exception("Code Update Error: " . $conn->error);
            ...
            $sql = "UPDATE tbl_cust_products_2025 
                    SET Division='$Division', Segment='$Segment', Family='$Family', ClassId='$ClassId', McDesc='$McDesc',
                        ...
                        MinQty='$MinQty', Photo='$Photo',checkstatus=1,push_flag=1, delete_flag=0
                    WHERE id='$id'";
            if (!$conn->query($sql)) throw new Exception("2025 Update Error: " . $conn->error);

            
            // ðŸŸ If product is inactive, update flags
            if ($Status == 0) {
                $flagQuery = "UPDATE tbl_cust_products_2025 
                              SET checkstatus=0, push_flag=1, delete_flag=1, modified_time='$modified_time'
                              WHERE id='$id'";
                if (!$conn->query($flagQuery)) throw new Exception("Flag Update Error: " . $conn->error);
            }
        }
        ...
        $redirect = empty($id) ? '../products/add-customer-product.php?user_id='.$userid : '../products/view-mrp-products.php?user_id='.$userid;
