...
// --------------------------------------
// WEEK WISE TOTAL SALES (Last 7 Days)
// --------------------------------------
$weekLabels = [];
$weekTotals = [];
for($i=6; $i>=0; $i--) {
    $dt = date('Y-m-d', strtotime("-$i days"));
    $weekLabels[] = date('d M', strtotime($dt));

    $sql = "
        SELECT COALESCE(SUM(NetAmount),0) AS Total
        FROM (
            ". unifiedInvoiceSQL("NetAmount", "AND InvoiceDate='$dt' AND FrId IN ($frids)") ."
        ) AS combined
    ";
    $weekTotals[] = getValue($sql);
}

// --------------------------------------
// MONTH WISE TOTAL SALES (Last 12 Months)
// --------------------------------------
$monthLabels = [];
$monthTotals = [];
...
for($i=11; $i>=0; $i--) {
    ...
    $sql = "
        SELECT COALESCE(SUM(NetAmount),0)
        FROM (
            ". unifiedInvoiceSQL("NetAmount", "AND InvoiceDate BETWEEN '$monthStart' AND '$monthEnd' AND FrId IN ($frids)") ."
        ) AS combined
    ";

    $monthTotals[] = getValue($sql);
}

// Send to JS
$weekSales  = ['labels'=>$weekLabels, 'totals'=>$weekTotals];
$monthSales = ['labels'=>$monthLabels, 'totals'=>$monthTotals];
...
<?php } ?>
    ...
    <div class="row g-3 mt-1">
      <div class="col-lg-6">
        <div class="card p-3 h-100">
          <h5 class="section-title mb-3"><i class="fa-solid fa-store me-2 text-primary"></i>Week Wise – Total Sales</h5>
          <canvas id="fofoBar" height="240" aria-label="COCO FOFO FICO FOCO Sales"></canvas>
        </div>
      </div>
      <div class="col-lg-6">
        <div class="card p-3 h-100">
          <h5 class="section-title mb-3"><i class="fa-solid fa-scale-balanced me-2 text-secondary"></i>Month Wise – Total Sales</h5>
...
const monthData = <?= json_encode($monthSales) ?>;
...
function openFilteredPopup(link) {
  ...
  let fromDate, toDate;
  const today = new Date();
  let frid = <?php echo $_REQUEST['user_id'];?>
...
const url = `${link.getAttribute('href')}?FromDate=${encodeURIComponent(fromDate)}&ToDate=${encodeURIComponent(toDate)}&calendar=${encodeURIComponent(calendar)}&user_id=${encodeURIComponent(frid)}`;
