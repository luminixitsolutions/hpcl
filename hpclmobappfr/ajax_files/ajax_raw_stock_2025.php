...
FROM tbl_cust_products_2025 AS tp
LEFT JOIN tbl_units AS tu ON tp.Unit = tu.Name
WHERE tp.ProdType = 0 AND tp.id = '$id'";
$row = getRecord($sql);
$Unit = $row['Name2'];
$Unit2 = $row['Unit'];
...
FROM `tbl_cust_prod_stock_2025` WHERE ProdId='$id' AND ProdType=0 AND FrId='$frid' GROUP by Status) as a";
	$row2 = getRecord($sql2);
	if($row2['balqty'] > 0){
