        ...
        <div class="main-container" style="padding-top: 80px;">
            <div class="container">
                ...
                <div class="card mb-4">
                   ...
                   <form id="validation-form" method="post" enctype="multipart/form-data" action="save-raw-wastge-stock.php">
                       <div class="card-body">
<div class="form-row">
...
<label class="form-label"> Raw Product<span class="text-danger">*</span></label>
 <select class="selectpicker form-control" name="ProdId" id="ProdId" onchange="getAvailProdStock(this.value)" style="width: 100%;padding-top: 12px;"  data-live-search="true">

<option selected="" value="">Select Product</option>
<?php 
$sql12 = "SELECT tp.ProductName,tp.id FROM tbl_cust_products_2025 tp WHERE tp.checkstatus=1 AND tp.CreatedBy='$FranchiseId' AND tp.ProdType='1' ORDER BY tp.ProductName";
  $row12 = getList($sql12);
...
<input type="file" name="Photo" id="Photo" class="form-control" placeholder="" value="" autocomplete="off" style="padding: 3px;" accept=".png">
<div class="clearfix"></div>
    </div>
...
function addToCart() {
    ...
    var PurchasePrice = $('#PurchasePrice').val();
    var SellPrice = $('#SellPrice').val(); // FIX: Added missing variable
    var TotalPrice = $('#TotalPrice').val();
    var QtyUnit = $('#QtyUnit').val();
    var photo = $('#Photo')[0].files[0]; // get the file
    ...
    if (ProdId === '') {
        ...
        formData.append("QtyUnit2", QtyUnit2);
        formData.append("QtyUnit", QtyUnit);
        formData.append("TotalPrice", TotalPrice); // FIX: Added missing TotalPrice

        if (photo) { // only append if a file is selected
        ...
        $.ajax({
            ...
            processData: false,
            beforeSend: function () {
                $('#add').attr('disabled', 'disabled').text('Please Wait...');
            },
            success: function (data) {
                ...
                // Reset fields
                $('#add').attr('disabled', false).text('Add');
                $('#code').val('');
                $('#ProdId').val('0').prop("selected", true);
                $('#Qty').val('');
                $('#PurchasePrice').val('');
                $('#AvailStock').val('');
                $('#Qty2').val('');
