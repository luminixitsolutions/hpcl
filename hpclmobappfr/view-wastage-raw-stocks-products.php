...
.product-info strong {
    color: #000;
                       ...
                       <?php 
$sql = "SELECT ts.StockDate, ts.Qty2, ts.Unit2, ts.id, ts.Photo, 
               p.ProductName, ts.PurchasePrice, ts.SellPrice 
        FROM tbl_cust_prod_stock_2025 ts 
        INNER JOIN tbl_cust_products_2025 p ON ts.ProdId = p.id 
        WHERE ts.FrId = '$FranchiseId' 
        AND ts.Status = 'Dr' 
        AND ts.ProdType = '1' 
        AND p.delete_flag = 0 
        AND ts.Wastage = 1";
...
foreach ($row as $result) {
    $image = !empty($result['Photo']) ? '../wastage_photos/raw/'.$result['Photo'] : 'no_image.jpg';
?>
            ...
            <div class="product-content">
                ...
                <div class="product-details">
                    ...
                    <p class="product-info">
                        Date: <strong><?php echo date("d/m/Y", strtotime($result['StockDate'])); ?></strong>
                    </p>
                    <p class="product-info">
                        Total Qty: <strong><?php echo $result['Qty2'] . " " . $result['Unit2']; ?></strong>
                    </p>
                </div>
            </div>
        </div>
