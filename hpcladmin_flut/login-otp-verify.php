...
<div class="otp-box">
    ...
    <form id="validation-form" method="post">
        ...
        <input type="hidden" name="login_action" value="OtpVerify">
        <input type="hidden" name="GetOtp" id="GetOtp" value="<?php echo htmlspecialchars((string)($_SESSION['otp'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>">
        <input type="hidden" name="Uid" id="Uid" value="<?php echo htmlspecialchars((string)($_GET['uid'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>">
        <input type="hidden" name="Phone" id="Phone" value="<?php echo htmlspecialchars((string)($_REQUEST['phone'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>">
...
function resendotp(){
    ...
    var sep = /\?/.test(base) ? '&' : '?';
    $.ajax({
        url: base + sep + 'login_action=resendotp&Phone=' + encodeURIComponent(Phone || ''),
        method: 'POST',
        data: {},
...
$(document).ready(function(){
    ...
    $('#validation-form').on('submit', function(e){
        ...
        var base = window.HPCL_AJAX_LOGIN || 'ajax_files/ajax_login.php';
        var sep = /\?/.test(base) ? '&' : '?';
        var url = base + sep + 'login_action=OtpVerify&Phone=' + encodeURIComponent(phone) + '&Uid=' + encodeURIComponent(uid) + '&YourOtp=' + encodeURIComponent(yourOtp);
        $.ajax({
            url: url,
            method: 'POST',
            data: {},
