...
if($_REQUEST["action"]=="delete")
{
    ...
    $conn->query($sql11);
    echo "<script>alert('Deleted Successfully!');window.location.href='view-other-products.php';</script>";
   /* }*/
...
<a href="add-other-product.php" class="btn btn-secondary btn-round"><i class="ion ion-md-add mr-2"></i> Add New</a></span>
<?php } ?>
</h4>
...
<th>Matching MRP Product</th>
        ...
        <tbody>
            <?php 
            $sql = "SELECT p.*,c.Name As Category,cs.Name As SubCatName,tp2.ProductName As MatchMrpProdName FROM tbl_cust_products2 p 
                    LEFT JOIN tbl_cust_category_2025 c ON c.id=p.CatId 
                    LEFT JOIN tbl_cust_sub_category_2025 cs ON cs.id=p.SubCatId  
                    LEFT JOIN tbl_cust_products2 tp2 ON tp2.id=p.MatchMrpProdId WHERE p.ProdType=0 AND p.ProdType2=3";
  ...
  <td><?php echo $row['id'];?></td>
              ...
              <td><?php echo $row['BarcodeNo']; ?></td>
                ...
                <td><?php echo $row['MatchMrpProdName'];?></td>
              ...
              <a href="add-other-product.php?id=<?php echo $row['id']; ?>"><i class="lnr lnr-pencil mr-2"></i></a>
            ...
            <?php if($user_id == 2003){?>
         <td><a onClick="return confirm('Are you sure you want delete this Product?\nNote : Delete all orders related this Product (Y/N)');" href="<?php echo $_SERVER['PHP_SELF']; ?>?id=<?php echo $row['id']; ?>&action=delete"><i class="lnr lnr-trash text-danger"></i></a></td>
            <?php } ?>
            </tr>
           <?php } ?>
        </tbody>
