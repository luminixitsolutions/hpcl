...
<head>
<title><?php echo $Proj_Title; ?> | Franchise Type</title>
<meta charset="utf-8">
...
<h4 class="font-weight-bold py-3 mb-0">List Of Dealer Type
...
<span class="font-weight-light">Franchise Type</span>
</h5>
...
<label class="form-label">Dealer Type Name <span class="text-danger">*</span></label>
...
function product_lists(){
      $.ajax({
  type: "POST",
  url: "ajax_files/ajax_franchise_type.php?action=view",
  data: {},
  dataType: "html",
  ...
  error: function(){
      $('#custresult').html('<div class="alert alert-danger">Could not load dealer types. Refresh the page.</div>');
  }
  });
    }
  function error_toast(){
   ...
   $.growl.error({
      title:    'Error',
      message:  'Franchise Type Already Exists',
   ...
   $.growl.success({
      title:    'Success',
      message:  'New Franchise Type Added Successfully!',
      location: isRtl ? 'tl' : 'tr'
    });
  }
function update_toast(){
   ...
   $.growl.success({
      title:    'Success',
      message:  'Franchise Type Updated Successfully!',
      location: isRtl ? 'tl' : 'tr'
    });
  }
  ...
  $(document).ready(function() {
      ...
      $('#add_button').click(function(){  
           $('.modal-title').html("Add <span class='font-weight-light'>Franchise Type</span>");  
           $('#action').val("Add");  
           $('#id').val('');
    ...
    if ($('#validation-form').valid()){ 
         $.ajax({  
                url :"ajax_files/ajax_franchise_type.php",  
                method:"POST",  
 ...
 $.ajax({  
                ...
                success:function(data){  
                    ...
                    $('#show_photo').html('<div class="ui-feed-icon-container float-left pt-2 mr-3 mb-3"><a href="javascript:void(0)" class="ui-icon ui-feed-icon ion ion-md-close bg-secondary text-white" id="delete_photo"></a><img src="../uploads/'+data.Photo+'" alt="" class="img-fluid ticket-file-img" style="width: 64px;height: 64px;"></div>');
                       ...
                       $('#submit').text("Update");   
                       $('.insert_frm').modal('show');
                         $('.modal-title').html("Update <span class='font-weight-light'>Franchise Type</span>"); 
 ...
 //alert(id);
   swal({
            title: "Are you sure?",
            text: "Deleted All Records Related this Franchise Type & You will not be able to recover this Franchise Type!",
            type: "warning",
            ...
            if (isConfirm) {
                ...
                success:function(data){
              swal("Deleted!", "Franchise Type has been deleted.", "success");
              ...
              product_lists();
                ...
                swal("Cancelled", "Franchise Type is safe :)", "error");
            }
        });
