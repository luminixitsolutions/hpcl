...
echo json_encode(array('MinPrice'=>$row['MinPrice'],'PurchasePrice'=>$row['PurchasePrice'],'code'=>$row['code']));
    ...
    $productByCode = $db_handle->runQuery("SELECT id,code,ProductName FROM tbl_cust_products_2025 WHERE id='" . $_POST["id"] . "'");
     $itemArray = array($productByCode[0]["code"]=>array('code'=>$productByCode[0]["code"],'id'=>$productByCode[0]["id"],'ProductName'=>$productByCode[0]["ProductName"],'PurchasePrice'=>$_POST["PurchasePrice"],'TotalPrice'=>$_POST["TotalPrice"],'Qty'=>$_POST["quantity"],'QtyUnit'=>$_POST["QtyUnit"],'Qty2'=>$_POST["Qty2"],'QtyUnit2'=>$_POST["QtyUnit2"],'MfgDate'=>$_POST["MfgDate"],'ExpDate'=>$_POST["ExpDate"]));
