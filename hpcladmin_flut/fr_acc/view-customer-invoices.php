...
<?php
            $sql = "SELECT SUM(NetAmount) AS NetAmount,SUM(SubTotal) AS SubTotal,SUM(Discount) AS TotDiscount FROM (SELECT  SUM(NetAmount) AS NetAmount,SUM(SubTotal) AS SubTotal,SUM(Discount) AS Discount, InvoiceDate FROM tbl_customer_invoice WHERE Roll=2 AND Status=1 AND FrId='$BillSoftFrId' AND delete_flag=0";
            ...
            $sql.=" UNION ALL SELECT SUM(NetAmount) AS NetAmount,SUM(SubTotal) AS SubTotal,SUM(Discount) AS Discount, InvoiceDate FROM tbl_customer_invoice_2025 WHERE Roll=2 AND Status=1 AND FrId='$BillSoftFrId' AND delete_flag=0";
            if($_POST['ReportType'] !=''){
                $ReportType = $_POST['ReportType'];
                if($ReportType == 'Today'){
                    $sql.=" AND InvoiceDate='".date('Y-m-d')."'";
...
<?php 
    function calAmt($type,$BillSoftFrId){
        global $conn;
         $sql = "SELECT SUM(NetAmount) AS NetAmount FROM (SELECT SUM(NetAmount) AS NetAmount FROM tbl_customer_invoice WHERE Roll=2 AND Status=1 AND FrId='$BillSoftFrId' AND PayType='$type' AND delete_flag=0";
         if($_POST['ReportType'] !=''){
                $ReportType = $_POST['ReportType'];
                if($ReportType == 'Today'){
                    $sql.=" AND InvoiceDate='".date('Y-m-d')."'";
            ...
            $sql.= " UNION ALL SELECT SUM(NetAmount) AS NetAmount FROM tbl_customer_invoice_2025 WHERE Roll=2 AND Status=1 AND FrId='$BillSoftFrId' AND PayType='$type' AND delete_flag=0";
            if($_POST['ReportType'] !=''){
                $ReportType = $_POST['ReportType'];
                if($ReportType == 'Today'){
                    $sql.=" AND InvoiceDate='".date('Y-m-d')."'";
...
<input type="text" class="form-control" value="&#8377;<?php echo $row['SubTotal']; ?>" autocomplete="off" readonly>
</div>

<div class="form-group col-md-2">
<label class="form-label">Discount</label>
<input type="text" class="form-control" value="&#8377;<?php echo $row['TotDiscount']; ?>" autocomplete="off" readonly>
</div>

<div class="form-group col-md-2">
<label class="form-label">Net Amount</label>
<input type="text" class="form-control" value="&#8377;<?php echo $row['NetAmount']; ?>" autocomplete="off" readonly>
</div>
...
<table id="example" class="table mb-0 dataTable no-footer" role="grid" aria-describedby="report-table_info">
        <thead class="thead-light">
            <tr>
              <!-- <th>#</th>-->
               ...
               <!--<th>Action</th>-->
              <?php if($AdminLoginId == 404 || $AdminLoginId == 2648 || $AdminLoginId == 2091) {?>
                <th>Cancel</th>
                <?php } else{?>
                <th></th>
                <?php } ?>
        ...
        <tbody>
            ...
            $sql = "SELECT *,'2024' AS Year FROM tbl_customer_invoice WHERE Roll=2 AND Status=1 AND FrId='$BillSoftFrId' AND NetAmount>0 AND delete_flag=0";
            ...
            $sql.= " UNION ALL SELECT *,'2025' AS Year FROM tbl_customer_invoice_2025 WHERE Roll=2 AND Status=1 AND FrId='$BillSoftFrId' AND NetAmount>0 AND delete_flag=0";
           ...
           // echo $sql;
            ...
            foreach($row12 as $result12){
                // $sql13 = "SELECT ProductName FROM  tbl_cust_products WHERE id='".$result12['ProdId']."' UNION ALL SELECT * FROM tbl_customer_invoice_details_2025 WHERE InvId='".$row['Unqid']."'";
...
$dados1 = json_encode(array('title'=>$title,'address'=>$Address,'mobile'=>$Phone,'gstin'=>$GstNo,'customer_name'=>$CustName,'customer_phone_no'=>$row['CellNo'],'receipt_title'=>'Retail#Receipt','invoice_no'=>$row['InvoiceNo'],'date'=>$date,'sub_total'=>$row['SubTotal'],'discount'=>$row['Discount'],'service_charge'=>0,'gst'=>$TotGst,'total_bill'=>$row['NetAmount'],'total_payable'=>$row['NetAmount'],'terms_condition'=>$terms_condition,'bottom_title'=>$bottom_title)); 
     ...
     $invoice_data =  json_encode(array_merge(json_decode($dados1, true),json_decode($dados2, true)));
            ...
            <tr>
             ...
             <?php if($AdminLoginId == 404 || $AdminLoginId == 2648 || $AdminLoginId == 2091) {?>                                         
              <td>
                 <a href="<?php echo $_SERVER['PHP_SELF']; ?>?id=<?php echo $row['id']; ?>&action=cancel&year=<?php echo $row['Year'];?>" onClick="return confirm('Are you sure you want cancel this invoice. delete all record related this invoice.');" class="btn btn-danger btn-sm btn-round"> Cancel</a>
             </td><?php } else{?>
             ...
             <td class="align-middle"><?php echo $row['SrNo']; ?></td>
