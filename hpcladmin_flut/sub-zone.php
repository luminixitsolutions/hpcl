...
.legend-color {
  ...
  border-radius: 50%;
    ...
    /* Dealers list modal — compact layout (no extra vertical gaps) */
    .dealers-modal.modal-content {
      padding: 0;
      width: 520px;
      max-width: 92vw;
      overflow: hidden;
    ...
    .dealers-modal__body .table {
      margin-bottom: 0;
    }
    .dealers-modal__body .table td,
    .dealers-modal__body .table th {
...
function subzone_invoice_date_clause($Calendar) {
    if ($Calendar == 'yesterday') {
        $Yesterday = date('Y-m-d', strtotime('-1 days'));
        return " AND InvoiceDate='$Yesterday'";
    }
    if ($Calendar == 'week') {
        $Week = date('Y-m-d', strtotime('-7 days'));
        return " AND InvoiceDate>='$Week' AND InvoiceDate<='".date('Y-m-d')."'";
    }
    if ($Calendar == 'month') {
        $Week = date('Y-m').'-01';
        return " AND InvoiceDate>='$Week' AND InvoiceDate<='".date('Y-m-d')."'";
    }
    if ($Calendar == 'Custom' || $Calendar == 'custom') {
        $p = '';
        if (!empty($_REQUEST['FromDate'])) {
            $FromDate = $_REQUEST['FromDate'];
            $p .= " AND InvoiceDate>='$FromDate'";
        }
        if (!empty($_REQUEST['ToDate'])) {
            $ToDate = $_REQUEST['ToDate'];
            $p .= " AND InvoiceDate<='$ToDate'";
        }
        return $p;
    }
    return " AND InvoiceDate='".date('Y-m-d')."'";
}

function subzone_dealer_net_amount($frid, $Calendar) {
    $frid = (int) $frid;
    if ($frid < 1) {
        return 0.0;
    }
    $d = subzone_invoice_date_clause($Calendar);
    $sql88 = "SELECT SUM(NetAmount) AS NetAmount FROM (SELECT SUM(NetAmount) AS NetAmount FROM tbl_customer_invoice WHERE FrId='$frid' ".$d
        ." UNION ALL SELECT SUM(NetAmount) AS NetAmount FROM tbl_customer_invoice_2025 WHERE FrId='$frid' ".$d.') AS a';
    $row88 = getRecord($sql88);
    return (float) ($row88['NetAmount'] ?? 0);
}
    
 if($Roll == 1){
     $sql = "SELECT * FROM tbl_sub_zone WHERE CatId='".$_GET['zoneid']."'";
 }
 else{
...
$row22 = getRecord($sql22);
        ...
        $zoneidq = $_GET['zoneid'];
        $sqlDealersList = "SELECT id, Fname, Lname, ShopName FROM tbl_users WHERE SubZoneId='$subzoneid' AND ZoneId='$zoneidq' AND Roll=5";
        if (!empty($shopAdminDealerIds = shopAdminDealerIds($row))) {
            $sqlDealersList .= " AND id IN (" . implode(',', $shopAdminDealerIds) . ")";
        }
        $sqlDealersList .= " ORDER BY ShopName ASC, Fname ASC";
        $dealerList = getList($sqlDealersList);
        $dealersModalRows = [];
        foreach ($dealerList as $dr) {
            $nm = trim($dr['ShopName']) !== '' ? trim($dr['ShopName']) : trim($dr['Fname'].' '.$dr['Lname']);
            $dealersModalRows[] = [
                'id' => (int) $dr['id'],
                'name' => $nm,
                'sale' => round(subzone_dealer_net_amount((int) $dr['id'], $Calendar), 2),
            ];
        }
        ?>
               <div class="col-lg-4">
...
<span style="font-size:12px;">
    Avg : <?php echo number_format((float)($avg ?? 0), 2); ?>
                            ...
                            <li class="list-group-item pt-2 pb-3" style="border-radius: 0;">
                                <button type="button" class="btn btn-sm btn-outline-primary btn-block" onclick="openDealersModal(this)" data-subzone="<?php echo htmlspecialchars($result['Name'], ENT_QUOTES, 'UTF-8'); ?>" data-dealers='<?php echo htmlspecialchars(json_encode($dealersModalRows), ENT_QUOTES, 'UTF-8'); ?>'>
                                    View Dealers
                                </button>
                            </li>
                        </ul>
...
<div id="dealersModal" class="modal">
  <div class="modal-content dealers-modal">
    <div class="dealers-modal__head">
      <div class="dealers-modal__title" id="dealersModalTitle">Dealers</div>
      <button type="button" class="dealers-modal__close" onclick="document.getElementById('dealersModal').style.display='none'" aria-label="Close">&times;</button>
    </div>
    <div class="dealers-modal__body" id="dealersModalBody"></div>
  </div>
</div>
...
window.onclick = function(e) {
  ...
  if (e.target === dealersModal) {
    dealersModal.style.display = 'none';
  }
};

function openDealersModal(btn) {
  const title = btn.getAttribute('data-subzone') || 'Sub zone';
  let dealers = [];
  try {
    dealers = JSON.parse(btn.getAttribute('data-dealers') || '[]');
  } catch (err) {
    dealers = [];
  }
  let html = '';
  if (!dealers.length) {
    html = '<p class="mb-0 text-muted">No dealers found.</p>';
  } else {
    html = '<table class="table table-sm table-striped mb-0"><thead><tr><th>Dealer</th><th class="text-right">Sale amount</th></tr></thead><tbody>';
    dealers.forEach(function (d) {
      const url = 'fr_acc/dashboard.php?id=' + encodeURIComponent(d.id);
      const name = escapeDealersHtml(d.name || '');
      const sale = Number(d.sale || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      html += '<tr><td><a href="' + url + '" target="_blank" rel="noopener">' + name + '</a></td><td class="text-right">&#8377; ' + sale + '</td></tr>';
    });
    html += '</tbody></table>';
  }
  document.getElementById('dealersModalTitle').innerText = title + ' — Dealers';
  document.getElementById('dealersModalBody').innerHTML = html;
  document.getElementById('dealersModal').style.display = 'flex';
}
function escapeDealersHtml(s) {
  if (!s) return '';
