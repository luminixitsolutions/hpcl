...
<head>
<title><?php echo $Proj_Title; ?> | Zone</title>
<meta charset="utf-8">
...
<h4 class="font-weight-bold py-3 mb-0">List Of Sub Zone
...
<?php 
        $q = "select * from tbl_zone WHERE Status='1'";
        $r = $conn->query($q);
...
function product_lists(){
      $.ajax({
  type: "POST",
  url: "ajax_files/ajax_sub_zone.php?action=view",
  data: {},
  dataType: "html",
  ...
  error: function(){
      $('#custresult').html('<div class="alert alert-danger">Could not load regions. Refresh the page.</div>');
  }
  });
    }
...
function update_toast(){
  ...
  $(document).ready(function() {
      ...
      $('#add_button').click(function(){  
           $('.modal-title').html("Add <span class='font-weight-light'>Zone</span>");  
           $('#action').val("Add");  
           $('#id').val('');

      $('#Name').val('');
      $('#CatId').attr("selected","selected").val(0);
      $('#Feedback').val('');
      $('#Photo').val('');
        $('#OldPhoto').val(''); 
        $('#show_photo').hide();
    ...
    if ($('#validation-form').valid()){ 
         $.ajax({  
                url :"ajax_files/ajax_sub_zone.php",  
                method:"POST",  
 ...
 $.ajax({  
                ...
                success:function(data){  
                     ...
                     $('#Name').val(data.Name);  
                      $('#CatId').val(data.CatId).attr("selected","selected");
                    ...
                    $('#show_photo').html('<div class="ui-feed-icon-container float-left pt-2 mr-3 mb-3"><a href="javascript:void(0)" class="ui-icon ui-feed-icon ion ion-md-close bg-secondary text-white" id="delete_photo"></a><img src="../uploads/'+data.Photo+'" alt="" class="img-fluid ticket-file-img" style="width: 64px;height: 64px;"></div>');
                       ...
                       $('#submit').text("Update");   
                       $('.insert_frm').modal('show');
                         $('.modal-title').html("Update <span class='font-weight-light'>Zone</span>"); 
 ...
 //alert(id);
   swal({
            title: "Are you sure?",
            text: "Deleted All Records Related this Zone & You will not be able to recover this Zone!",
            type: "warning",
            ...
            if (isConfirm) {
                ...
                success:function(data){
              swal("Deleted!", "Zone has been deleted.", "success");
              ...
              product_lists();
                ...
                swal("Cancelled", "Zone is safe :)", "error");
            }
        });
