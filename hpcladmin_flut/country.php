...
<h4 class="font-weight-bold py-3 mb-0">List Of Country
...
<input type="text" name="Name" class="form-control" id="Name" placeholder="Country Name" required>
...
function category_lists(){
      $.ajax({
  type: "POST",
  url: "ajax_files/ajax_country.php?action=view",
  data: {},
  dataType: "html",
  ...
  error: function(){
      $('#custresult').html('<div class="alert alert-danger">Could not load countries. Refresh the page.</div>');
  }
  });
    }
...
function update_toast(){
  ...
  $(document).ready(function() {
      ...
      $('#add_button').click(function(){  
           $('.modal-title').html("Add <span class='font-weight-light'>Country</span>");  
           $('#action').val("Add");  
           $('#id').val('');
    ...
    if ($('#validation-form').valid()){ 
         $.ajax({  
                url :"ajax_files/ajax_country.php",  
                method:"POST",  
                ...
                success:function(data){  
                    ...
                    $('#Status').val(data.Status).attr("selected",true);  
                     ...
                     $('#action').val('Edit'); 
                       ...
                       $('#submit').text("Update");   
                       $('.insert_frm').modal('show');
                         $('.modal-title').html("Update <span class='font-weight-light'>Country</span>"); 
