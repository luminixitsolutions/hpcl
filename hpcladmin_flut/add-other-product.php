...
$row7 = getRecord($sql7);
?>
<form action="ajax_files/ajax_customer_other_products.php" method="POST" enctype="multipart/form-data" autocomplete="off">
...
<?php 
        $q = "select * from tbl_cust_category_2025 WHERE Status='1' AND ProdType=2";
        $r = $conn->query($q);
        while($rw = $r->fetch_assoc())
    {
...
<?php 
        $q = "select * from tbl_cust_sub_category_2025 WHERE Status='1' AND CatId='".$row7['CatId']."' AND ProdType=2";
        $r = $conn->query($q);
        while($rw = $r->fetch_assoc())
    {
...
<label class="form-label"> Matching MRP Product </label>
 <select class="select2-demo form-control" name="MatchMrpProdId" id="MatchMrpProdId">

<option selected="" value="0">Select Product</option>
<?php 
  $sql12 = "SELECT id,ProductName,MinPrice FROM tbl_cust_products2 WHERE ProdType=0 AND ProdType2=1 AND Status=1";
  $row12 = getList($sql12);
  foreach($row12 as $result){
     ?>
  <option <?php if($row7["MatchMrpProdId"] == $result['id']) {?> selected <?php } ?> value="<?php echo $result['id'];?>">
