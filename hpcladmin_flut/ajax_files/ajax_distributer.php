...
if ($_POST['action'] == 'checkVendorPincode') {
    ...
    if (!empty($pincodes)) {
        if($id!=''){
            $sql = "SELECT id, AssignPincode FROM tbl_users_bill WHERE VedId='$VedId' AND id!='$id' AND Roll=166 AND Status=1";
        }
        else{
          $sql = "SELECT id, AssignPincode FROM tbl_users_bill WHERE VedId='$VedId' AND Status=1 AND Roll=166";  
        }
       // echo $sql;
