...
<head>
<title><?php echo $Proj_Title; ?> | City</title>
<meta charset="utf-8">
...
<h4 class="font-weight-bold py-3 mb-0">List Of City
...
<label class="form-label">Top City <span class="text-danger">*</span></label>
  <select class="form-control" id="TopCity" name="TopCity" required="">
...
function category_lists(){
      $.ajax({
  type: "POST",
  url: "ajax_files/ajax_city.php?action=view",
  data: {},
  dataType: "html",
  ...
  error: function(){
      $('#custresult').html('<div class="alert alert-danger">Could not load cities. Refresh the page.</div>');
  }
  });
    }
...
function update_toast(){
   ...
   var action = "getState";
    ...
    success:function(data)
    {
      ...
      $('#add_button').click(function(){  
           $('.modal-title').html("Add <span class='font-weight-light'>City</span>");  
           $('#action').val("Add");  
           $('#id').val('');
      ...
      $('#TopCity').attr("selected","selected").val(0);
 ...
 $.ajax({  
                ...
                success:function(data){  
                    ...
                    $('#TopCity').val(data.TopCity).attr("selected",true); 
                 ...
                 getState(CountryId,StateId); 
                     ...
                     $('#action').val('Edit'); 
                       ...
                       $('#submit').text("Update");   
                       $('.insert_frm').modal('show');
                         $('.modal-title').html("Update <span class='font-weight-light'>City</span>"); 
