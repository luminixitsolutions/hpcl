...
<h4 class="font-weight-bold py-3 mb-0">List Of State
...
<span class="font-weight-light">State</span>
</h5>
...
<input type="text" name="Name" class="form-control" id="Name" placeholder="State Name" required>
<div class="clearfix"></div>
</div>
</div>
...
function category_lists(){
      $.ajax({
  type: "POST",
  url: "ajax_files/ajax_state.php?action=view",
  data: {},
  dataType: "html",
  ...
  error: function(){
      $('#custresult').html('<div class="alert alert-danger">Could not load states. Refresh the page.</div>');
  }
  });
    }
...
function update_toast(){
  ...
  $(document).ready(function() {
      ...
      $('#add_button').click(function(){  
           $('.modal-title').html("Add <span class='font-weight-light'>State</span>");  
           $('#action').val("Add");  
           $('#id').val('');
                ...
                success:function(data){  
                 ...
                 $('#CountryId').val(data.CountryId).attr("selected",true);  
                     $('#action').val('Edit'); 
                       ...
                       $('#submit').text("Update");   
                       $('.insert_frm').modal('show');
                         $('.modal-title').html("Update <span class='font-weight-light'>State</span>"); 
