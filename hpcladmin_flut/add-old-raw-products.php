...
if($_GET['id']==''){
    ...
    $qx="INSERT INTO tbl_cust_products2 SET PurchasePrice='$PurchasePrice',MinQty='$MinQty',Qty='$Qty',ProductName = '$ProductName',Status='$Status',CreatedDate='$CreatedDate',CreatedBy='$BillSoftFrId',code='$Code',Unit='$Unit',ProdType='1',Transfer=1,SubCatId='$SubCatId',CatId='$CatId',OldNew='$OldNew'";
  $conn->query($qx);
  ...
  echo "<script>alert('Raw Product Added Successfully!');window.location.href='view-old-raw-products.php';</script>";
}
else{
 

    $query2 = "UPDATE tbl_cust_products2 SET PurchasePrice='$PurchasePrice',MinQty='$MinQty',Qty='$Qty',ProductName = '$ProductName',Status='$Status',ModifiedDate='$CreatedDate',Unit='$Unit',ProdType='1',SubCatId='$SubCatId',CatId='$CatId',OldNew='$OldNew' WHERE id = '$id'";
  $conn->query($query2);
...
$conn->query($sql);
  ...
  echo "<script>alert('Raw Product Updated Successfully!');window.location.href='view-old-raw-products.php';</script>";
...
<h4 class="font-weight-bold py-3 mb-0">Old Raw Product</h4>
...
<?php 
        $q = "select * from tbl_cust_category_2025 WHERE Status='1' AND ProdType=1 AND OldNew=0";
        $r = $conn->query($q);
...
<label class="form-label">Old <span class="text-danger">*</span></label>
  <select class="form-control" id="OldNew" name="OldNew" required="">

<option value="1" <?php if($row7["OldNew"]=='1') {?> selected <?php } ?>>New</option>
